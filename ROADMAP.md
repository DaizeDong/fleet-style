...
- **File types `dash_guard` does not examine.** The extension map covers Markdown, plain prose, Python and templates. Anything outside it is skipped, and a skipped file is reported but not checked, so prose in a YAML description or a shell comment passes.
